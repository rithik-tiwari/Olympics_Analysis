...
def data_over_time(df,col):
    ...
    nation_over_time = nation_over_time['Year'].value_counts().reset_index().sort_values(by='count')
    nation_over_time.rename(columns={'count':col,'Year':'Years'},inplace = True)
...
def countryYear_wise_medalTally(df,country):
    temp_df = df.dropna(subset=['Medal'])
...
def most_successful_countrywise(df, country):
    ...
    x = x.merge(df, left_on='Name', right_on='Name', how='left')[['Name','index','Sport']].drop_duplicates('Name')
    x.rename(columns={'index':'Medals'},inplace = True)
    return x
