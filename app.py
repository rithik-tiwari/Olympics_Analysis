...
if user_menu == 'Overall Analysis':
    ...
    fig = px.line(nations_over_time, x="Years", y="region")
    st.title("Participating Nations Over the Years.")
    st.plotly_chart(fig)

    events_over_time = helper.data_over_time(df,'Event')
    fig = px.line(events_over_time, x="Years", y="Event")
    st.title("Events Over the Years.")
    st.plotly_chart(fig)

    athletes_over_time = helper.data_over_time(df,'Name')
    fig = px.line(athletes_over_time, x="Years", y="Name")
...
if user_menu == 'Country-wise Analysis':
    ...
    country_df = helper.countryYear_wise_medalTally(df,selected_country)
    fig = px.line(country_df, x="Year", y="Medal")
    st.title(selected_country + "'s Medal Tally over the Years.")
    st.plotly_chart(fig)

    st.title(selected_country + "'s Sports Wise Medal Tally over the Years.")
    fig,ax = plt.subplots(figsize=(20,20))
...
if user_menu == 'athlete wise analysis':
    ...
    famous_sports = ['Basketball','Judo', 'Football', 'Tug-Of-War', 'Athletics', 'Swimming', 'Badminton', 'Sailing', 'Gymnastics',
                ...
                'Baseball', 'Rhythmic Gymnastics', 'Rugby Sevens', 'Beach Volleyball', 'Triathlon', 'Rugby',
                 'Polo',  'Ice Hockey', ]
    ...
    st.title("Men vs Wemen Participation over the  Years.")
